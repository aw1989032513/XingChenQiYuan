...
#define KKKINTEGSDK_DEPRECATED(_version) __attribute__((deprecated("接口已不推荐,版本号:"#_version)))
typedef enum {
 ...
 *  登陆回调token,用于登陆验证
 */
@property (nonatomic,copy)NSString *loginToken KKKINTEGSDK_DEPRECATED(3.9);

/**
 *  登陆回调签名,用于登陆验证,游戏客户端需传给游戏服务端  v3.9
 */
@property (nonatomic,copy)NSString *kkk_sign;

/**
 *  登陆回调时间戳，用于登陆验证,游戏客户端需传给游戏服务端  v3.9
 */
@property (nonatomic,copy)NSString *kkk_time;


/**
 *  登陆回调玩家年龄  v4.2.4
 */
@property (nonatomic,copy)NSString *kkk_age;
