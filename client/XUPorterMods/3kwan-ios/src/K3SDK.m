...
-(id) init{
    if(self = [super init]){
        ...
        // 实名验证结果通知
        [[NSNotificationCenter defaultCenter] addObserver: self selector: @selector(realNameResult:) name: k3KIntegratedSDKVerifyRealNameResultNotification object: nil];
...
- (void) SdkInit:(NSString*) config{
    ...
    [info setAppId: @"348"];
    [info setAppKey: @"FsvonfS4WkqiqloZMee3F1IcLILAR9UY"];
    [info setMerchantId: @""];
    [info setMd5Key: @""];
    [info setGameIdFor3K: @"194"];
    [info setGameNameFor3K: @"星辰奇缘（子包10）-（3K服）"];
    // SDK的UI是否允许自动旋转
...
-(void) StatisticsInfo:(NSString*) info{
    ...
    NSString *roleCTime = [json objectForKey:@"roleCTime"];
...
//     NSLog(@"统计信息: roldId:%@ roleName:%@ roleLevel:%@ serverId:%@ serverName:%@ vipLevel:%@ balance:%@ roleCTime:%@ typeString:%@", roleId, roleName, roleLevel, serverId, serverName, vipLevel, balance, roleCTime, typeString);
...
- (void)logoutCallBack: (NSNotification *)notification {
    ...
    // 通知游戏逻辑处理
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setObject:@"change_account" forKey:@"type"];
    [dict setObject:@"" forKey:@"error"];
    [self sendMessage: dict];
...
-(void)realNameResult:(NSNotification *)notification {
    NSDictionary *noticeDic = notification.userInfo;
    NSLog(@"用户实名年龄:%@", noticeDic[@"age"]);
}
@end
